...
<body>
    <a href="Lab-4-Ex2.php"><button type="button">Return</button></a>
    <br><br>
    <?php
        $file_size = array();// Lưu danh sách các file dưới dạng {key => value} = {filename => filesize}
        $file_time = array();// Lưu danh sách các file dưới dạng {key => value} = {filename => filetime}
        $directory = 'upload/';
        ...
        foreach ($list0 as $key => $value) {
            ...
            $file_time[$value] = date("Y-m-d H:i:s", filemtime($path));// filemtime: lấy thời gian chỉnh sửa mới nhất
        }
